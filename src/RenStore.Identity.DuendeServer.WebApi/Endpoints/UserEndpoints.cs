...
public static class UserEndpoints 
{
    public static IEndpointRouteBuilder MapUserEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/register", RequestRegister);

        group.MapPost("/login", RequestLogin);

        group.MapPost("/logout", Logout);
        
        group.MapPost("/verify-email-and-login", VerifyEmailAndLogin);
    ...
    private static async Task<IResult> RequestRegister(
        ...
        IEmailVerificationService emailVerificationService)
    {
        ...
        if (result)
        {
            await userService.ConfirmEmail(request.Email);
            return Results.Ok();
        }
    ...
    private static async Task<IResult> RequestLogin(
        ...
        UserService userService)
    {
        ...
        {
            await userService.ConfirmEmail(request.Email!);
            return Results.Ok(new { message = "Verification code sent to email!"});
        }
        
        return Results.BadRequest("Invalid credentials! Your email or password is incorrect.");
    }
    ...
    private static async Task<IResult> VerifyEmailAndLogin(
        VerifyEmailRequest request,
        UserService userService)
    {
        var result = await userService
            .VerifyEmail(request.Email, request.Code);
    ...
    // TODO: сделать автоматическуб отправку кода на емайл после регистрации, логина
